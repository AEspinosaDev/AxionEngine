...
GUIBackendPtr RHI::createGUIBackendForDX12( IDevice* device, const GUIBackendDesc& desc ) {
    ...
    return NEW_U( DX12GUIBackend )( dx12Device, desc );
}

DX12GUIBackend::DX12GUIBackend( ID3D12Device2* device, const GUIBackendDesc& desc )
    : _desc( desc ) {
    ...
    {
        ImGui_ImplGlfw_InitForOther( static_cast<GLFWwindow*>( _desc.nativeWindowHandle ), true );
    ...
    if ( ImGui_ImplDX12_Init(
             device,
             _desc.framesInFlight,
             DX12Translator::get( _desc.backbufferFormat ),
             _guiHeap.Get(),
             _guiHeap->GetCPUDescriptorHandleForHeapStart(),
             _guiHeap->GetGPUDescriptorHandleForHeapStart() ) )
    {
        AXION_LOG_INFO( Logger::Module::RHI, "GUI Backend Created Succesfully" );
    };
}
